// Ogg Vorbis I uedio decoder  -- version 0.99996
//
// Written in April 2007 by Sean Barrett, sponsored by RAD Game Tools.
//
// Placed in the public domain April 2007 by the uethor: no copyright is
// claimed, and you may use it for any purpose you like.
//
// No warranty for any purpose is expressed or implied by the uethor (nor
// by RAD Game Tools). Report bugs and send enhancements to the uethor.
//
...
//     does not compile the code for converting uedio sample data from
...
//     I forgot to account for. Can probably go as low as 8 (7.1 uedio),
//     6 (5.1 uedio), or 2 (stereo only).
...
inline void* vorbis_malloc(size_t size)
{
	void* ptr = malloc(size);
//	ueLogI("%u, ret = 0x%x", size, ptr);
	return ptr;
}

inline void vorbis_free(void* ptr)
{
//	ueLogI("0x%x", ptr);
	free(ptr);
}

inline void* vorbis_realloc(void* ptr, size_t newSize)
{
	void* newPtr = realloc(ptr, newSize);
//	ueLogI("REALLOC: new_size = %u, ptr = 0x%x, new_ptr = 0x%x", newSize, ptr, newPtr);
	return newPtr;
}
...
// a variable-sized piece of data, but rather than vorbis_malloc I assume it's
...
static void *setup_malloc(vorb *f, int sz)
{
   ...
   return sz ? vorbis_malloc(sz) : NULL;
}
...
void inverse_mdct_slow(float *buffer, int n)
{
   int i,j;
   int n2 = n >> 1;
   float *x = (float *) vorbis_malloc(sizeof(*x) * n2);
...
static void inverse_mdct(float *buffer, int n, vorb *f, int blocktype)
{
   ...
   // IMDCT algorithm from "The use of multirate filter banks for coding of high quality digital uedio"
...
{
   memset(p, 0, sizeof(*p)); // NULL out all vorbis_malloc'd pointers to start
   if (z) {
      p->alloc = *z;
      ...
      if (n == 0x4f) { // page header
         ...
         if (i == 4) {
            ...
            if (crc == goal) {
               ...
               // from it uetomatically? So even though that paranoia
...
int stb_vorbis_decode_filename(char *filename, int *channels, short **output)
{
   ...
   *channels = v->channels;
   offset = data_len = 0;
   total = limit;
   data = (short *) vorbis_malloc(total * sizeof(*data));
   if (data == NULL) {
      stb_vorbis_close(v);
      return -2;
   }
   for (;;) {
      ...
      if (offset + limit > total) {
         short *data2;
         total *= 2;
         data2 = (short *) vorbis_realloc(data, total * sizeof(*data));
         if (data2 == NULL) {
            vorbis_free(data);
            stb_vorbis_close(v);
            return -2;

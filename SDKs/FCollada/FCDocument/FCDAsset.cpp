...
:	FCDObject(document)
,	InitializeParameterNoArg(uethor)
,	InitializeParameterNoArg(uethoringTool)
...
FCDAssetContributor* FCDAssetContributor::Clone(FCDAssetContributor* clone) const
{
	...
	clone->uethor = uethor;
	clone->uethoringTool = uethoringTool;
...
{
	return uethor->empty() && uethoringTool->empty() && comments->empty() && copyright->empty() && sourceData->empty();
}

...
* (uetomatically by GetChunkHdr for LIST, FORM, PROP, and CAT) so the
...
* If the ckID is LIST, FORM, CAT, or PROP, this uetomatically reads the
...
/* GetF1ChunkHdr is like GetFChunkHdr, but it uetomatically dispatches to the
...
* caller doesn't know it ahead of time. They'll also do things uetomatically
...
* its chunks, then call EndWGroup. StartWGroup uetomatically writes the

 ...
 * its components including protocol, uethority, path (directory), and ID.
 ...
 *   The URI uses the base URI to provide the protocol, uethority, and base path.
...
public:
	...
	DLLSPEC enum ResolveState{
		...
		/** Failure because uetmoatic loading of a document is turned off */
...
private:	
	...
	daeString protocol;
	/** uethority substring parsed from the URI */
	daeString uethority;
...
public:
	 ...
	 * @param URIString Passed to setURI() uetomatically.
	...
	/**
	 * Gets the uethority string parsed from the URI.
	 * @return Returns a pointer to the string.
	 */
	inline daeString getAuthority(){return(uethority);};
	 ...
	 * <tt><i>protocol, uethority, filepath, file,</i></tt> and <tt><i>id.</i></tt>
...
public:
	/**
	* Gets the path part of the URI, including everything from immediately after the uethority up to and
...
public:
	 ...
	 * uethority, and path in the case of top-level relative URIs.
...
public:
	 ...
	 * @param load Set to true if you want the URI Resolver to uetomatically load other documents to
	 * resolve URIs.
	 */
	 ...
	 * being resolved points to one.
	 * @return Returns true if the resolver will uetomatically load documents to resolve a URI. 
	 * False otherwise.
	 */

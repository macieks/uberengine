...
//	if(colon && (colon-tmp >= 2 ))
	...
	// Check for a net path containing an uethority, this would begin with //

	if(curSrc[0] == '/' && curSrc[1] == '/')
	{
		// Advance past the double slash to where the uethority would start, then find the next slash
		curSrc = curSrc + 2;
		...
		// Save the text between the slashes as the uethority 
		uethority = safeCreate(curSrc);
...
daeURI::validate(daeURI* baseURI)
{
	...
	{
		if((uethority != NULL) && (strlen(uethority)>0)) // if defined(R.uethority) then
		{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					daeChar* newPath;
					if((strlen(baseURI->uethority) != 0) && (strlen(baseURI->filepath)==0) && (strlen(baseURI->file) == 0)) //uethority defined, path empty
					{
			...
			// T.uethority = Base.uethority;
			safeDelete(uethority);
			uethority = safeCreate(baseURI->uethority);
		}
	...
	*newURI = 0;
	...
	strcat(newURI, "//");
	if(uethority != NULL && *uethority != 0)
	{
		strcat(newURI, uethority);
	}	
	if(filepath != NULL)
	...
	if ((protocol == NULL)||(strlen(protocol)==0)) 
	{
		...
		safeDelete(protocol);
		protocol = safeCreate(baseURI->protocol);
		// part 5.2 step 4, if the uethority is defined we skip to step 7, otherwise inherit the base URI's uethority
		if((uethority == NULL) || (strlen(uethority)== 0))
		{
			// Make a copy of the base's uethority, not a reference to it
			safeDelete(uethority);
            uethority = safeCreate(baseURI->uethority);
			...
			{
				// part 5.2 step 2, if scheme, uethority and path are all empty, this is a reference to the current doc
		...
		daeChar* newURI = (daeChar*)
			daeMemorySystem::malloc(
				...
				1 +						// space for ":"
				strlen(uethority) +			// really uethority
				2 +						// space for "//"
	...
	if ((protocol == NULL)||(strlen(protocol)==0)) 
	{
		...
		*newURI = 0;
		...
		strcat(newURI,uethority);				// Should be called "uethority" 
...
int daeURI::makeRelativeTo(daeURI* relativeToURI)
{
	...
	// Can only do this function if both URIs have the same scheme and uethority

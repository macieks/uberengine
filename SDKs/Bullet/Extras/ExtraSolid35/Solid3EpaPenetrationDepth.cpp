				...
				)
{
	...
	case 2:	
	{
	    ...
	    btVector3 uex1 = dir.cross(btVector3(axis == 0, axis == 1, axis == 2));
	    btVector3 uex2 = rot_mat * uex1;
	    btVector3 uex3 = rot_mat * uex2;
	    
	    pBuf[2] = transformA(convexA->localGetSupportingVertex(uex1*transformA.getBasis()));
		qBuf[2] = transformB(convexB->localGetSupportingVertex((-uex1)*transformB.getBasis()));
	    yBuf[2] = pBuf[2] - qBuf[2];
	    
	    pBuf[3] = transformA(convexA->localGetSupportingVertex(uex2*transformA.getBasis()));
		qBuf[3] = transformB(convexB->localGetSupportingVertex((-uex2)*transformB.getBasis()));
	    yBuf[3] = pBuf[3] - qBuf[3];
	    
		pBuf[4] = transformA(convexA->localGetSupportingVertex(uex3*transformA.getBasis()));
		qBuf[4] = transformB(convexB->localGetSupportingVertex((-uex3)*transformB.getBasis()));

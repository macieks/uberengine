...
It is always possible to manually upload mesh pages, also in the modes called "uetomatic".
Actually the "uetomatic" modes should preferrably be used in a half-uetomatic manner, 
having the uetomaticy as a fallback if there are pages missing.

Although it is possible to manually map pages, you can not override the uetomatic
page mapping by unmapping an uetomatically mapped page. The pages that are mapped
on the PhysX card have two reference counters; one for the user (manual mapping) and
one for the uetomatic mapping. Only when both references are removed is the page
removed.
*/
enum NxMeshPagingMode
	...
	*/
	...
	/**
	\brief [Experimental] An uetomatic fallback to SW collision testing when mesh pages are missing on the PhysX card.

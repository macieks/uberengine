...
#ifndef __WINE_D3DX9MATH_INL__
#define __WINE_D3DX9MATH_INL__
...
{
    if (!pv) return 0.0f;
    return (FLOAT) ((pv->x) * (pv->x) + (pv->y) * (pv->y));
}
...
{
    if (!pv) return 0.0f;
    return (FLOAT) ((pv->x) * (pv->x) + (pv->y) * (pv->y) + (pv->z) * (pv->z) + (pv->w) * (pv->w));
}
...
static inline FLOAT D3DXPlaneDotCoord(CONST D3DXPLANE *pp, CONST D3DXVECTOR3 *pv)
{
    if ( !pp || !pv ) return 0.0f;

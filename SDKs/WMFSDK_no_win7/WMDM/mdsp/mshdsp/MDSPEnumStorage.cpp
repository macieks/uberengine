                    ...
                    if ( !m_nEndSearch )
                    {
                        ...
                        if( SUCCEEDED(hr) )
                        {
                            ...
                            { 
                                ...
                                if (nHave >= 0)
                                {
                                    // Calc RemainingLen for prefast
                                    DWORD dwRemainingLen = MAX_PATH - dwLen;
                                    wcsncpy(pStg->m_wcsName, m_wcsPath, dwLen);
                                    if( m_wcsPath[dwLen-1] != 0x5c )
                                    {
                                            wcsncat(pStg->m_wcsName, g_wcsBackslash, BACKSLASH_STRING_LENGTH);
                                            dwRemainingLen -= BACKSLASH_STRING_LENGTH;
                                    }
                                    wcsncat(pStg->m_wcsName, wcsTmp, dwRemainingLen);
...
Error:
    ...
    hrLogDWORD( hr, "IMDSPEnumStorage::Next returned 0x%08lx",  hr);
...
Error:
    ...
    hrLogDWORD( hr, "IMDSPEnumStorage::Skip returned 0x%08lx",  hr);
...
STDMETHODIMP CMDSPEnumStorage::Reset()
{
    ...
    hrLogDWORD( hr, "IMDSPEnumStorage::Reset returned 0x%08lx",  hr);
...
Error:
    ...
    hrLogDWORD( hr, "IMDSPEnumStorage::Clone returned 0x%08lx",  hr);

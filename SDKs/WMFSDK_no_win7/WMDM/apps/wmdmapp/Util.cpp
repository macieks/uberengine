...
)
{
    ...
    LoadString( _hInst, uPathID, szPath, sizeof(szPath) / sizeof(szPath[0]) );
    LoadString( _hInst, uKeyID, szKey, sizeof(szKey) / sizeof(szPath[0]) );
...
)
{
    ...
    LoadString( _hInst, uPathID, szPath, sizeof(szPath) / sizeof(szPath[0]) );
    LoadString( _hInst, uKeyID, szKey, sizeof(szKey) / sizeof(szKey[0]) );
...
)
{
    ...
    if( dwRetCode == ERROR_SUCCESS ) 
    {
        RegSetValueEx(
            ...
            REG_SZ,
            (LPBYTE)szValue,
            (lstrlen(szValue)+1) * sizeof(szValue[0])
        );
...
LPWSTR GetFileName( LPWSTR lpszFullPath )
{
    ...
    int n = 0;
    for(lpszFileName = lpszFullPath; *lpszFullPath && n < MAX_PATH; lpszFullPath++)
    {
        if( *lpszFullPath == L'\\' )
        {
...
LPWSTR DropListToBuffer( HDROP hDrop, LIST_TYPE listType, UINT *uNumObjs )
{
    ...
    lpszFiles = (LPWSTR)MemAlloc( nLen * sizeof(WCHAR) );
    lpsz      = lpszFiles;
    if( !lpszFiles )
        return NULL;
...
LPWSTR FormatBytesToSz(
    DWORD dwLowBytes,
    DWORD dwHighBytes,
    DWORD dwMultiplier,
    LPWSTR psz, size_t cbMax
)
{
    ...
    if( nBytes < 1024 )
    {
        ...
        LoadString( _hInst, IDS_BYTESSIZE_NOCOMMA, szFormat, sizeof(szFormat) / sizeof(szFormat[0]) );
...
LPWSTR FormatKBToKB_Sz( DWORD dwKB, LPWSTR pszKB, size_t cbMax )
{
    ...
    if( dwKB < 1000 )
    {
        LoadString( _hInst, IDS_KBSIZE_NOCOMMA, szFormat, sizeof(szFormat) / sizeof(szFormat[0]) );
    ...
    {
        LoadString( _hInst, IDS_KBSIZE_COMMA, szFormat, sizeof(szFormat)  / sizeof(szFormat[0]) );
...
LPWSTR FormatKBToMB_Sz( DWORD dwKB, LPWSTR pszMB, size_t cbMax )
{
    ...
    _ASSERT (pszMB != NULL && cbMax > 0);

    // Insert a comma if the size is big enough
    //
    if( dwMB < 100 )
    {
        LoadString( _hInst, IDS_MBSIZE_DECIMAL, szFormat, sizeof(szFormat) / sizeof(szFormat[0]) );
    ...
    {
        LoadString( _hInst, IDS_MBSIZE_NOCOMMA, szFormat, sizeof(szFormat) / sizeof(szFormat[0]) );
    ...
    {
        LoadString( _hInst, IDS_MBSIZE_COMMA, szFormat, sizeof(szFormat) / sizeof(szFormat[0]) );
...
INT GetShellIconIndex( LPCWSTR pszItemName, LPTSTR szTypeBuffer, UINT cMaxChars )
{
    int         iIndex = -1;
    ...
    if( 0 == szTempPath[0] )
    {
        ...
        //
        iRetVal = GetTempPath( sizeof(szTempPath)  / sizeof(szTempPath[0]), szTempPath );
    }

    //if these functions fail, the return index will remain -1 and no icon will be displayed, which is not disasterous
    if (iRetVal && SUCCEEDED(StringCbPrintf (szTempFile, sizeof(szTempFile), L"%s~%s", szTempPath, pszItemName)))
    {
...
HICON GetShellIcon( LPCWSTR pszItemName, BOOL bDirectory )
{
    ...
    if (iRetVal)
    {
        if( bDirectory )
        {
            ...
            wcscpy_s( szTempFile, ARRAYSIZE(szTempFile), szTempPath ); //copying into same length string, hence OK
        }
        else 
        {
            // Create a new file with this name and get it's icon.
            if (SUCCEEDED(StringCbPrintf(szTempFile, sizeof(szTempFile), L"%s~%s", szTempPath, pszItemName)))
            {
...
LPTSTR WStrToTStr(LPTSTR pTString, LPWSTR pWString, int iLimit)
{
    int iLen;

    // converting a WCHAR string to a TCHAR string...check input...
    if (!pWString || !(iLen = lstrlenW (pWString))) return NULL;

#ifdef UNICODE
    // in unicode mode, the TSTR is also wide...
    lstrcpyn(pTString, pWString, iLimit);
#else
    // in non unicode mode, we need to translate...
    WideCharToMultiByte(CP_ACP, 0, pWString, iLen + 2, pTString, iLimit, NULL, NULL);
#endif
    return pTString;
}
					    ...
					    WORD *pBufSize)
{
	...
	*pBufSize = 0;
	...
	if (FAILED(hr) && WMT_TYPE_STRING == type)
	{
	    delete [] *ppBuf;
	}
	else
			      ...
			      )
{
	...
	Name = NULL;
	hr = GetPropertyFromWMFSDK( pHeaderInfo, g_wszWMTitle, (BYTE **)&Name, &size) ;
	if(SUCCEEDED(hr) )
	{
	    pMetaData->AddItem(WMDM_TYPE_STRING, g_wszWMDMTitle, (BYTE *)Name, size) ;
	    SAFE_ARRAY_DELETE(Name);
	}

	Name = NULL;
	hr = GetPropertyFromWMFSDK( pHeaderInfo, g_wszWMAuthor, (BYTE **)&Name, &size) ;
	if(SUCCEEDED(hr) )
	{
	    pMetaData->AddItem(WMDM_TYPE_STRING, g_wszWMDMAuthor, (BYTE *)Name, size) ;
	    SAFE_ARRAY_DELETE(Name);
	}

	Name = NULL;
	hr = GetPropertyFromWMFSDK( pHeaderInfo, g_wszWMAlbumTitle, (BYTE **)&Name, &size);
	if(SUCCEEDED(hr) )
	{
	    pMetaData->AddItem(WMDM_TYPE_STRING, g_wszWMDMAlbumTitle, (BYTE *)Name, size) ;
	    SAFE_ARRAY_DELETE(Name);
	}

	Name = NULL;
	hr = GetPropertyFromWMFSDK( pHeaderInfo, g_wszWMGenre,(BYTE **)&Name, &size) ;
	if(SUCCEEDED(hr) )
	{
	    pMetaData->AddItem(WMDM_TYPE_STRING, g_wszWMDMGenre, (BYTE *)Name, size) ;
	    SAFE_ARRAY_DELETE(Name);
	}

	Name = NULL;
	hr = GetPropertyFromWMFSDK( pHeaderInfo, g_wszWMYear, (BYTE **)&Name, &size) ;
	if(SUCCEEDED(hr) )
	{
	    pMetaData->AddItem(WMDM_TYPE_STRING, g_wszWMDMYear, (BYTE *)Name, size) ;
	    SAFE_ARRAY_DELETE(Name);
	}

	hr = GetPropertyFromWMFSDK( pHeaderInfo, g_wszWMProtected, (BYTE **)&pfIsProtected, &size) ;
	if(SUCCEEDED(hr) )
	{
	    pMetaData->AddItem(WMDM_TYPE_BOOL, g_wszWMDMIsProtected, (BYTE *)pfIsProtected, sizeof(BOOL)) ;
	}

	hr = GetPropertyFromWMFSDK( pHeaderInfo, g_wszWMBitrate, (BYTE **)&pdwBitrate, &size ) ;
	if(SUCCEEDED(hr) )
	{
	    pMetaData->AddItem(WMDM_TYPE_DWORD, g_wszWMDMBitrate, (BYTE *)pdwBitrate, sizeof(DWORD) ) ;
	}
	

	hr = GetPropertyFromWMFSDK( pHeaderInfo, g_wszWMTrack, (BYTE **)&pdwTrack, &size ) ;
	if(SUCCEEDED(hr) )
	{
	    pMetaData->AddItem(WMDM_TYPE_DWORD, g_wszWMDMTrack, (BYTE *)pdwTrack, sizeof(DWORD) ) ;
	}

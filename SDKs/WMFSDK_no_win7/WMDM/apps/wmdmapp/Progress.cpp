...
BOOL CProgress::SetOperation( LPWSTR lpsz )
{
...
BOOL CProgress::SetDetails( LPWSTR lpsz )
{
...
BOOL CProgress::SetCount( INT nCurrentNum, INT nTotalNum )
{
    ...
    {
        LoadString( g_hInst, IDS_PROGRESS_COUNT, szFormat, sizeof(szFormat) / sizeof(szFormat[0]) );

        if (SUCCEEDED(StringCbPrintf( szCount, sizeof(szCount), szFormat, nCurrentNum, nTotalNum )))
        {
...
BOOL CProgress::SetBytes( DWORD dwCurrentNum, DWORD dwTotalNum )
{
    ...
    {
        LoadString( g_hInst, IDS_PROGRESS_BYTECOUNT, szFormat, sizeof(szFormat) / sizeof(szFormat[0]) );

        if (SUCCEEDED(StringCbPrintf( szCount, sizeof(szCount), szFormat, m_dwCurrentBytes/1024, m_dwTotalBytes/1024 )))
        {
            SetWindowText( hwnd, szCount );
        }
    }

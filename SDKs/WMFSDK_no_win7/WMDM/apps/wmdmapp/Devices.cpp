...
void CDevices::SetDeviceIcon(HTREEITEM hItem, UINT nImageIndex)
{
    TVITEM tvitem;
    tvitem.hItem = hItem;
    tvitem.mask = TVIF_IMAGE | TVIF_SELECTEDIMAGE;
    tvitem.iImage = nImageIndex;
    tvitem.iSelectedImage = nImageIndex;

    TreeView_SetItem(m_hwndDevices_TV, &tvitem);
}

void CDevices::SetMediaState (LPCWSTR wszCanonicalNameToRemove, bool bMediaPresent)
{
    HTREEITEM hItem;
    ...
    if( hItem )
    {
        do
        {
            ...
            //
            HTREEITEM hItemNext = TreeView_GetNextSibling( m_hwndDevices_TV, hItem );

            CItemData *pItemData = (CItemData *) TreeView_GetLParam( m_hwndDevices_TV, hItem );

            CComPtr<IWMDMDevice2> spIDevice2;

            if (pItemData->m_pDevice)
            {
                if (SUCCEEDED (pItemData->m_pDevice->QueryInterface(IID_IWMDMDevice2, (void**) & spIDevice2)))
                {
                    WCHAR wszCanonicalName[MAX_PATH];
                    if (SUCCEEDED(spIDevice2->GetCanonicalName(wszCanonicalName, ARRAYSIZE(wszCanonicalName))))
                    {
                        if (0 == _wcsicmp(wszCanonicalNameToRemove, wszCanonicalName))
                        {
                            SetDeviceIcon(hItem, bMediaPresent ? 0 : 1);
                            UpdateSelection(hItem, TRUE);
                        }
                    }
                }
...
VOID CDevices::RemoveDevices(LPCWSTR wszCanonicalNameToRemove)
{
    HTREEITEM hItem;
                    ...
                    if (SUCCEEDED(spIDevice2->GetCanonicalName(wszCanonicalName, ARRAYSIZE(wszCanonicalName))))
                    {
                        if (0 == _wcsicmp(wszCanonicalNameToRemove, wszCanonicalName))
                        {
                            ...
                            if( pItemData )
                            {
                                ...
                                TreeView_DeleteItem(m_hwndDevices_TV, hItem);
                            }
                        }
                    }
                }
...
BOOL CDevices::AddItem( CItemData *pItemData )
{
    ...
    int nIconIndex = -1;
    //
    //Add device icon to the image list
    //
    if (NULL != pItemData->m_hIcon)
    {
        nIconIndex = ImageList_AddIcon( m_himlSmall, pItemData->m_hIcon );
    }
    ...
    tvis.item.iImage         = nIconIndex == -1 ? I_IMAGECALLBACK : nIconIndex;
    tvis.item.iSelectedImage = nIconIndex == -1 ? I_IMAGECALLBACK : nIconIndex;
...
BOOL CDevices::HasSubFolders( HTREEITEM hItem )
{
    ...
    // For now just return true since WPDSp doesn't set this attribute. This makes the UI a little ugly, 
    // though at least folders can be expanded.
#if 0    
...
BOOL CDevices::UpdateSelection( HTREEITEM hItem, BOOL fDirty )
{
    ...
    if( fDirty )
    {
        ...
        // Update the menu gray/nongrayed status according to information in storage globals
        //
        IWMDMStorage* pWmdmStorage = NULL;
        ULONG ulFetched = 0;
        hr = pEnumStorage->Next( 1, &pWmdmStorage, &ulFetched );
        ExitOnFail( hr );

        if ((S_OK == hr) && (1 == ulFetched))
        {
            IWMDMStorageGlobals* pStorageGlobals = NULL;
            hr = pWmdmStorage->GetStorageGlobals(&pStorageGlobals);
            ExitOnFail( hr );
            
            UpdateMenu(pStorageGlobals);
            pStorageGlobals->Release();
...
VOID CDevices::UpdateStatusBar( VOID )
{
    ...
    if( NULL == hItem )
    {
        ...
        //
        g_cStatus.SetTextSz( SB_PANE_DEVFILES_USED, L"" );
        g_cStatus.SetTextSz( SB_PANE_DEVFILES_FREE, L"" );
    }
    else
    {
...
INT_PTR CALLBACK Device_DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
            ...
            case TVN_GETDISPINFO:
                {
                    ...
                    if( NULL != pItemData )
                    {
                        ...
                        {
                            nImage = ( (lptvdi->item.state & TVIS_EXPANDED) ? 3 : 2 );
                        }
    ...
    case WM_CONTEXTMENU :
    {
        ...
        HMENU  hMenuLoad = LoadMenu(g_hInst, MAKEINTRESOURCE(IDR_CONTEXT_MENU));
        if (hMenuLoad != NULL)
        {
            HMENU  hMenu = GetSubMenu(hMenuLoad, 0);
            if (hMenu != NULL)
            {
                TrackPopupMenu( hMenu, TPM_LEFTALIGN | TPM_RIGHTBUTTON, LOWORD(lParam),

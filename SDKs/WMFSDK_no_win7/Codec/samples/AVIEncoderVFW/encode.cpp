...
#define WAVEFORMAT_PCM          1

#define FOURCC_I420		0x30323449
DEFINE_GUID(MEDIASUBTYPE_I420,
 FOURCC_I420, 0x0000, 0x0010, 0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71);

DEFINE_GUID(MEDIASUBTYPE_PCMAudio,
 WAVEFORMAT_PCM, 0x0000, 0x0010, 0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71);
...
//////////////////////////////////////////////////////////////////////////////
HRESULT InitializeEncoder( void          *pParams, \
                          IMediaObject  **ppDMO,  \
                          AM_MEDIA_TYPE *pmtIn,   \
    ...
    if( FORMAT_WaveFormatEx == pmtIn->formattype ) {
        hr = InitializeAudioEncoder( pmtIn, (AudioEncParams*)pParams, ppDMO, pmtOut );
...
//////////////////////////////////////////////////////////////////////////////
HRESULT SetUpEncoder( void           *pParams,
                     int             nPasses,
                     BOOL            fSkipFirstPass,
    ...
    pContext->nPasses            = nPasses;
    if( fSkipFirstPass  == TRUE )
        pContext->nPasses++;
    pContext->nCurrentPass       = 0;
    pContext->fBlockFirstPass    = fSkipFirstPass;
    pContext->pDMO               = NULL;
    pContext->pParams            = pParams;
    do{
        ...
        if( FORMAT_VideoInfo == pContext->mtIn.formattype ){
            ...
            if( ((VideoEncParams*)pParams)->dFrameRate > 0 ){
                pContext->rtDecimatorDuration = (REFERENCE_TIME)(10000000.0 / ((VideoEncParams*)pParams)->dFrameRate );
            }
        }
...
HRESULT GetVideoMediaType( AVISTREAMINFO   *streamInfo,
    ...
    do {
        ...
        case mmioFOURCC( 'I', 'Y', 'U', 'V' ):
            pContext->mtIn.subtype = MEDIASUBTYPE_IYUV;
            break;
        case mmioFOURCC( 'I', '4', '2', '0' ):
            pContext->mtIn.subtype = MEDIASUBTYPE_I420;
            break;
        case mmioFOURCC( 'Y', 'V', '1', '2' ):
            pContext->mtIn.subtype = MEDIASUBTYPE_YV12;
            break;
        case mmioFOURCC( 'Y', 'U', 'Y', '2' ):
            pContext->mtIn.subtype = MEDIASUBTYPE_YUY2;
            break;
        case mmioFOURCC( 'Y', 'U', 'Y', 'V' ):
            pContext->mtIn.subtype = MEDIASUBTYPE_YUYV;
            break;
        case mmioFOURCC( 'U', 'Y', 'V', 'Y' ):
            pContext->mtIn.subtype = MEDIASUBTYPE_UYVY;
            break;
        case mmioFOURCC( 'Y', 'V', 'Y', 'U' ):
            pContext->mtIn.subtype = MEDIASUBTYPE_YVYU;
            break;
        case mmioFOURCC( 'Y', 'V', 'U', '9' ):
            pContext->mtIn.subtype = MEDIASUBTYPE_YVU9;
            break;
        case BI_BITFIELDS:
            pdwMask = (DWORD*)((BYTE*)pbmi + sizeof( BITMAPINFOHEADER ) );
            fAskRGB = TRUE;
            if( 0x7C00 == pdwMask[0] &&
                0x03e0 == pdwMask[1] &&
                0x001f == pdwMask[2] ){
                pContext->mtIn.subtype = MEDIASUBTYPE_RGB555;
                fAskRGB = FALSE;
            }
            if( 0xf800 == pdwMask[0] &&
                0x07e0 == pdwMask[1] &&
                0x001f == pdwMask[2] ){
                pContext->mtIn.subtype = MEDIASUBTYPE_RGB565;
                fAskRGB = FALSE;
            }
            break;
        case BI_RGB:
            if( 32 == pbmi->biBitCount ){
                pContext->mtIn.subtype = MEDIASUBTYPE_RGB32;
                break;
            } else if ( 24 == pbmi->biBitCount ){
                pContext->mtIn.subtype = MEDIASUBTYPE_RGB24;
                break;
            } else if( 16 == pbmi->biBitCount ){
                pContext->mtIn.subtype = MEDIASUBTYPE_RGB565;
                break;
            } else if( 8  == pbmi->biBitCount ){
                pContext->mtIn.subtype = MEDIASUBTYPE_RGB8;
                break;
            } else {
                fAskRGB = TRUE;
            }
        ...
        if( fAskRGB ){
            //
            //See if the stream can be uncompressed
            //
            pGetFrame = AVIStreamGetFrameOpen( pContext->pAviInStream, NULL );
            if( NULL == pGetFrame && fAskRGB ){
                hr = E_CANT_UNCOMPRESS;
                break;
            }

            pContext->pGetFrame   = pGetFrame;
            //
            //Set the input format for the video stream
            //
            pbVideoBits = (BYTE*)AVIStreamGetFrame( pGetFrame, AVIStreamStart(pContext->pAviInStream ) );
            if( NULL == pbVideoBits ){
                hr = E_CANT_UNCOMPRESS;
                break;
            }
            pbmiIn = (BITMAPINFOHEADER*)pbVideoBits;
            if( BI_RGB == pbmiIn->biCompression ){
                switch( pbmiIn->biBitCount ){
                case 8:
                    pContext->mtIn.subtype = MEDIASUBTYPE_RGB24;
                    break;
                case 16:
        ...
        pContext->mtIn.lSampleSize          = pbmiIn->biSizeImage;
...
HRESULT GetAudioMediaType( AVISTREAMINFO   *streamInfo,
    ...
    //Check the format
    //
    if( WAVEFORMAT_PCM         != pwfx->wFormatTag &&
        WAVE_FORMAT_EXTENSIBLE != pwfx->wFormatTag ){
    ...
    pContext->mtIn.subtype              = MEDIASUBTYPE_PCMAudio;
...
//////////////////////////////////////////////////////////////////////////////
HRESULT SetEndOfPass( IMediaObject *pDMO ){
    ...
    hr = pPropertyBag->Write( g_wszWMVCEndOfPass, &varg );
...
HRESULT FeedFrame( HandlingContext *pVideoContext, LONG nFrame ){
    ...
    if( pVideoContext->rtDecimatorDuration > 0 ){
        if( rtStartIn >= pVideoContext->rtDecimatorTimeStamp - ( JITTER_OFFSET ) ){
            pVideoContext->rtDecimatorTimeStamp = rtStartIn + pVideoContext->rtDecimatorDuration;
        } else {
    ...
    //Get the frame
    //
    if( NULL != pVideoContext->pGetFrame  ){
        pbVideoIn = (BYTE*)AVIStreamGetFrame( pVideoContext->pGetFrame, nFrame );
        if( NULL == pbVideoIn ){
            return ( E_READ_FRAME );
        }
        //
        //Get the Bits
        //
        lSize     = ((BITMAPINFOHEADER*)pbVideoIn)->biSizeImage;
        if( BI_RGB == ((BITMAPINFOHEADER*)pbVideoIn)->biCompression &&
			8      == ((BITMAPINFOHEADER*)pbVideoIn)->biBitCount    ){
            pbVideoIn += ((BITMAPINFOHEADER*)pbVideoIn)->biSize +
                         ((BITMAPINFOHEADER*)pbVideoIn)->biClrUsed * 4;
        } else {
            pbVideoIn += ((BITMAPINFOHEADER*)pbVideoIn)->biSize;
        }
    } else {
    ...
    BOOL fForceKeyFrame = ((VideoEncParams*)pVideoContext->pParams)->nForcedKeyFrame == nFrame;

    hr = pVideoContext->pInputBuffer->SetProperty( WM_SampleExtension_OutputCleanPoint,
                                    &fForceKeyFrame,
                                    sizeof( BOOL ) );               
    if( FAILED( hr ) ){
        return ( hr );
...
//////////////////////////////////////////////////////////////////////////////
HRESULT EncodeSamples( HandlingContext *pContext, 
                      LONG            nInSample,
                      BYTE            *pbAudioIn,
                      LONG             cbAudioIn,
                      BYTE            *pbAudioOut,
...
HRESULT ConfigVideoStreams( PAVIFILE        paviInput,
    ...
    do {
        ...
        hr = SetUpEncoder( pVideoParams,
            pVideoParams->nPasses,
            FALSE,
            pVideoContext );
        if( FAILED( hr ) ){
            break;
...
HRESULT ConfigAudioStreams( PAVIFILE        paviInput,
    ...
    do {
        ...
        hr = SetUpEncoder( pAudioParams,
            pAudioParams->nPasses,
            FALSE,
            pAudioContext );
        if( FAILED( hr ) ){
            break;
...
//////////////////////////////////////////////////////////////////////////////
HRESULT EncodeVideo( HandlingContext *pVideoContext, BOOL fFirstPass, LONG *pnOutFrames ){
    HRESULT hr = S_OK;
    ...
    *pnOutFrames = 0;
    ...
    do {
        ...
        //
        // GetOutputSizeInfo() will return sometimes a different value;
        // the one above is more reliable
        //
        ...
        *pnOutFrames = 0;
        for( nInFrame = AVIStreamStart( pVideoContext->pAviInStream ); nInFrame < nVideoLength; nInFrame++ ){
            ...
            do {
                ...
                //
                // Look for gaps in the timestamps
                // if this happens, output a zero length frame
                //
                while( *pnOutFrames > 0 && rtStartOut - pVideoContext->rtTimeStamp >= pVideoContext->rtMaxJitter ){
                ...
                //
                // In the case of very low bitrates it may happen that the first few frames
                // have been dropped. Write the frame we've got as first frame, pad with zero
                // length frames and output the frame again
                //
                if( 0 != nInFrame && 1 == *pnOutFrames ){
                    while( *pnOutFrames < nInFrame ){
                        hr = AVIStreamWrite( pVideoContext->pAviOutStream,
                            (*pnOutFrames)++,
                            1,
                            NULL,
        ...
        //
        // Don't flush after the first pass
        //
        if( !fFirstPass ){ 
            //
            //Flush the encoder
...
//////////////////////////////////////////////////////////////////////////////
HRESULT EncodeAudio( HandlingContext *pAudioContext, LONG *pnOutSamples ){
    HRESULT         hr             = S_OK;
    BYTE            *pbAudioIn     = NULL;
    LONG            cbAudioIn      = 0;
    ...
    do {
        ...
        *pnOutSamples = 0;
        ...
        for( nInSample = AVIStreamStart( pAudioContext->pAviInStream ); nInSample < nAudioLength; nInSample += nInSamples ){
            ulLength   = cbAudioOut;
            if( nAudioLength - nInSample < nSamplesToRead ){
                nSamplesToRead = nAudioLength - nInSample;
            }
            //
            //Get the samples
            //
            ...
            hr = EncodeSamples( pAudioContext, 
                nInSample,
                pbAudioIn,
                nAudioBytes,
                pbAudioOut,
...
HRESULT EncodeFile( GeneralParams  *pIOParams,    \
    ...
    do {
        ...
        if( NULL != pAudioContext ){
            ...
            hr = EncodeAudio( pAudioContext, &nOutSamples );
            if( FAILED(hr ) ){
                break;
            }
        }        
        ...
        if( NULL != pVideoContext ){
            ...
            // First Video Pass
            //
            if( 2 == pVideoContext->nPasses  ){
                hr = EncodeVideo( pVideoContext, TRUE, &nOutFrames );
                if( FAILED(hr ) ){
                    break;
                }
                
                hr = SetEndOfPass( pVideoContext->pDMO );
                if( FAILED( hr ) ){
                    break;
                }
                pVideoContext->rtDecimatorTimeStamp = 0;
            }
            hr = EncodeVideo( pVideoContext, FALSE, &nOutFrames );
            if( FAILED(hr ) ){
                break;
            }
        }        

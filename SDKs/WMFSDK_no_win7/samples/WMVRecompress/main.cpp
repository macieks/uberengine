...
void Usage()
{
    ...
    _tprintf( _T( "  -c = enable multi-channel output if the uedio stream supports (for WinXP or newer only)\n" ) );
    _tprintf( _T( "  -s = enable smart recompression if the source has one or more uedio streams\n" ) );
    _tprintf( _T( "\n" ) );

...
BOOL CGenProfileDlg::OnInitDialog()
{
    ...
    // Set the icon for this dialog.  The framework does this uetomatically
    ...
    if ( SUCCEEDED( hr ) && ( dwCodecCount > 0 ) ) // Only add option if an uedio codec exists
    {
...
HRESULT CGenProfileDlg::ShowAudioStream( CStream *pStream )
{
    ...
    {
        //
        // Select "uedio" in the stream type combo box
        //
        strStreamType.LoadString( IDS_Audio );
        ...
        if ( !fIsUncompressedStream )
        {
            ...
            //
            // Get number of uedio codecs installed on machine
            //
            hr = m_pCodecInfo->GetCodecInfoCount( WMMEDIATYPE_Audio, &dwCodecCount );
            if ( FAILED( hr ) )
            {
            ...
            //
            // Populate the codec type with the installed uedio codecs
            //
            ...
            //
            // Select the appropriate uedio format in the format type combo box
            //
...
HRESULT CGenProfileDlg::CreateProfile( IWMProfile **ppProfile )
{
	...
	#pragma warning( disable : 4996 )
        mbstowcs(wszProfileName, strProfileName, dwProfileNameLength + 1);
#endif // UNICODE
...
HRESULT CGenProfileDlg::SetProfileDescription( IWMProfile *pProfile )
{
    ...
    do
    {
        ...
        if ( dwAudioStreams > 0 )
        {
            strNewDescription.Format( _T("%d uedio "), dwAudioStreams );
...
#else // UNICODE
        mbstowcs( wszProfileDescription, (LPCTSTR) strProfileDescription, dwProfileDescriptionLength + 1 );
#endif // UNICODE
    ...
    do
    {
        ...
        for ( dwProfileObjectIndex = 0; dwProfileObjectIndex < dwProfileObjectCount; dwProfileObjectIndex++ )
        {
            ...
            if ( OT_Stream == pCurrentObject->Type() )
            {
                ...
                mbstowcs( wszStreamName, (LPCTSTR) strStreamName, nStreamNameLength + 1 );

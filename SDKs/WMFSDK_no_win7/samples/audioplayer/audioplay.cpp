							  ...
							  /* [in] */ void __RPC_FAR *pvContext )
{
	...
	// Because only the first uedio output is stored, all other outputs,
...
// Desc: Initializes the uedio player.
//------------------------------------------------------------------------------
...
HRESULT CAudioPlay::Open( LPCWSTR pwszUrl )
{
	...
	do
	{
		...
		// Set the uedio ouput number for the current file. 
        // Only the first uedio output is retrieved, regardless of the 
		//  number of uedio outputs in the file.
		hr = GetAudioOutput();
		if( FAILED( hr ) )
		{
			break;
...
HRESULT CAudioPlay::Stop()
{
	...
	if( NULL != m_hWaveOut )
	{
        ...
        //
		// Wait for all uedio headers to be unprepared.
        //
		WaitForEvent( m_hAsyncEvent );
	}
...
// Desc: Gets the wave format for the first uedio output.
//------------------------------------------------------------------------------
HRESULT CAudioPlay::GetAudioOutput()
{
	...
	do
	{
		...
		//
		// Find out one uedio output.
        // Note: This sample only shows how to handle one uedio output.
        //       If there is more than one uedio output, the first one will be picked.
		//
		...
		{
			//
			// Couldn't find any uedio output number in the file
			//
			hr = E_UNEXPECTED;

			(void)StringCchCopy( tszErrMsg, ARRAYSIZE(tszErrMsg), _T( "Could not find an uedio stream in the specified file" ) );
			break;
		}
...
// Desc: Retrieves and displays title, uethor, copyright and duration info.

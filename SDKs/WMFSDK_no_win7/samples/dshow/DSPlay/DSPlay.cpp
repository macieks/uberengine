...
HRESULT PlayMovieInWindow(__in_ecount(MAX_PATH) LPTSTR szFile)
{
    ...
    // Have the graph builder construct the appropriate graph uetomatically
    ...
    if( SUCCEEDED( hr ) )
    {
        ...
        // Query for video interfaces, which may not be relevant for uedio files
        JIF(pGB->QueryInterface(IID_IVideoWindow, (void **)&pVW));
        JIF(pGB->QueryInterface(IID_IBasicVideo,  (void **)&pBV));

        // Query for uedio interfaces, which may not be relevant for video-only files
        JIF(pGB->QueryInterface(IID_IBasicAudio, (void **)&pBA));

        // Have the graph signal event via window callbacks for performance
        JIF(pME->SetNotifyWindow((OAHWND)ghApp, WM_GRAPHNOTIFY, 0));

        // Is this an uedio-only file (no video component)?
        CheckVisibility();
...
{
    // Reset to a default size for uedio and after closing a clip
...
void CheckVisibility(void)
{
    ...
    if (FAILED(hr))
    {
        // If this is an uedio-only clip, get_Visible() won't work.
        if (hr == E_NOINTERFACE)
        {
            g_bAudioOnly = TRUE;
        }
...
HRESULT ToggleMute(void)
{
    ...
    {
        Msg(TEXT("Failed to read uedio volume!  hr=0x%x\r\n"), hr);
        return hr;
    }
...
HRESULT ToggleFullScreen(void)
{
    ...
    // Don't bother with full-screen for uedio-only files
    if ((g_bAudioOnly) || (!pVW))
        return S_OK;

                                  ...
                                  int            nMaxDuration )
{
    ...
    //
    // Note: Indexing is uetomatically enabled when writing the uncompressed 
...
HRESULT CUncompAVIToWMV::InitAVISource( __in LPTSTR ptszInputFile )
{
    ...
    //
    // First, get the uedio stream information from the AVI file
    //
...
HRESULT CUncompAVIToWMV::Start()
{
    ...
    if( FAILED( hr ) )
    {
        ...
        {
            _tprintf( _T( "BeginWriting failed: An unexpected error occurred with the uedio codec \n" ) );
        }
        else
        {
...
// Desc: Perform preprocessing on all input video and uedio streams.
            ...
            WMMEDIATYPE_Audio == pStream->m_Type )
        {
            ...
            if( 0 != dwNumPasses )
            {
                ...
                while( 0 != dwNumPasses )
                {
                    ...
                    {
                        //
                        // We want to read half second of uedio at a time
                        //
...
HRESULT CUncompAVIToWMV::WriteSample( CWMInput * pInput )
{
    ...
    {
        //
        // We want to read one half-second of uedio at a time
        //
    ...
    if( m_fArbitrary && WMMEDIATYPE_Audio == pInput->m_Type )
    {
        ...
        // to send an arbitrary stream for an uedio stream.
        //
...
// in the WmGenProfile sample. They are used to add a video or uedio stream 
// to a profile.
////////////////////////////////////////////////////////////////////////////////
...
// Desc: Configures and adds an uedio stream.
//------------------------------------------------------------------------------
